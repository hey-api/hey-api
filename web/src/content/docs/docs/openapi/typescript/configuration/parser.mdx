...
When an operation defines multiple media types for a request body, response, or parameter, we select one of them. By default, we prefer a JSON media type, then the first defined entry.

You can control this selection with `content.preferred` — an ordered list of media types. Each entry is matched in order against the available media types; the first match wins. Strings match the media type exactly, ignoring parameters such as `; charset=utf-8` and casing. Regular expressions are tested against the full media type. When nothing matches, we fall back to the default behavior, so setting this option never changes output for operations it doesn't match.

<Tabs>
  <TabItem label="requests">
```js {6-10} title="openapi-ts.config.ts"
export default {
  input: 'hey-api/backend', // sign up at app.heyapi.dev
  output: 'src/client',
  parser: {
...
```
  ...
  <TabItem label="everywhere">
```js {6-8} title="openapi-ts.config.ts"
export default {
  input: 'hey-api/backend', // sign up at app.heyapi.dev
  output: 'src/client',
  parser: {
...
The array shorthand applies to requests, responses, and parameters. Use the object form to configure each context separately with the `requests`, `responses`, and `parameters` keys.
