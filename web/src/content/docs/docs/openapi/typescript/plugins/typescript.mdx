...
If you consume the generated types with your own HTTP client, you may want a single index of every operation instead of hand-enumerating the per-operation types. Set `operations` to `true` to export a type keyed by operation ID.
...
export type Operations = {
  addPet: {
    data: AddPetData;
    responses: AddPetResponses;
  };
};
```

The `errors` and `responses` members are present only when the operation defines them. If your specification defines a schema named `Operations`, the index type is renamed automatically. You can also pick the name yourself by passing a string, e.g. `operations: 'MyOperations'`.
