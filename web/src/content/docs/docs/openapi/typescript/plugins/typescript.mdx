...
The `errors` and `responses` members are present only when the operation defines them. You can rename the type by passing a string, e.g. `operations: 'MyOperations'`.
