...
If you consume the generated types with your own HTTP client, you may want each request type to carry its HTTP method. Set `requests.method` to `true` to include the method as a literal type.
...
:::note
If you pair this option with a custom client copied from an older template, update its `Options` type to omit the new field: `Omit<TData, 'url' | 'method'>`.
:::
