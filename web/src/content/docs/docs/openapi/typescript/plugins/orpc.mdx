---
title: oRPC Plugin
description: Generate oRPC v1 or v2 contracts from OpenAPI with the oRPC plugin for openapi-ts. Fully compatible with validators, transformers, and all core features.
sidebar:
  label: oRPC
custom:
  version: v1/v2
...
To generate oRPC v2 contracts, set `version` to `'v2'`.
...
When generating oRPC v2 contracts, `queryStyles` metadata is inferred by default so OpenAPI query parameters keep their original serialization behavior.
